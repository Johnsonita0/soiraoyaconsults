...
export default function HeroSlider({ slides, content, onContact }) {
  ...
  useEffect(() => { setImageSources(safeSlides.map((slide) => slide.image)); setActiveIndex((index) => (index >= safeSlides.length ? 0 : index)) }, [safeSlides])
  useEffect(() => { const timer = window.setInterval(() => setActiveIndex((index) => (index + 1) % safeSlides.length), 6500); return () => window.clearInterval(timer) }, [safeSlides.length])
  const move = (direction) => setActiveIndex((index) => (index + direction + safeSlides.length) % safeSlides.length)
